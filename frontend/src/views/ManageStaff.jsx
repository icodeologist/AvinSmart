...
export default function ManageStaff() {
  ...
  function editStaff(member) {
    setSelectedStaff(member);
    setEditing(true);
    setEditForm({ name: member.name, email: member.email, phone: member.phone, role: member.role, status: member.status });
    setPassword("");
    setNotice("");
  }
  ...
  return (
    <>
      <PageHeader title="Manage Staff" subtitle="View your staff members">
        <Link to="/pos/login" className="btn btn-success"><i className="ti ti-device-desktop me-1"></i>Open Staff POS</Link>
